...
export default function TableApproved() {
  ...
  const handleFilterDocumentType = async (e) => {
    ...
    let {data} = await supabase.from('request').select('*').eq("document_type", e.target.value).eq('status', 'Approved');
    SetDocuments(data)
  }

  useEffect(() => {
    const fetchData = async () => {
      let {data} = await supabase.from('request').select('*').eq('status', 'Approved');
