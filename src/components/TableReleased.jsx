...
export default function TableReleased() {
  ...
  const handleFilterDocumentType = async (e) => {
    ...
    let {data} = await supabase.from('request').select('*').eq("document_type", e.target.value).eq('status', 'Released');
    SetDocuments(data)
  }

  useEffect(() => {
    const fetchData = async () => {
      let {data} = await supabase.from('request').select('*').eq('status', 'Released');
